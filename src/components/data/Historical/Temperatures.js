...
export default function Temperatures(props) {
  ...
  const min = Math.min(...min_a.filter(x => x))
  const min_i = min_a.indexOf(min)
  ...
  let avg = null
  if (avg_a.length > 0 ) { avg = avg_a.reduce((sum,x) => sum+x) / avg_a.length }
